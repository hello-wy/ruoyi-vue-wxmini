...
class LecturesServiceImplTest {
    ...
    @Test
    void selectAllLecturesListVoDelegatesWithoutTimeWindowFiltering() {
        Lectures filter = new Lectures();
        when(lecturesMapper.selectAllLecturesListVo(filter)).thenReturn(Collections.emptyList());

        service.selectAllLecturesListVo(filter);

        verify(lecturesMapper).selectAllLecturesListVo(filter);
    }
