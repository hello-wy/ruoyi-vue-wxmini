...
- 微信转账创建成功后，只要提现记录进入 `status=0` 打款中，就立即扣减钱包可用余额并写入支出流水；不是等 `status=1` 已到账后才扣款。
- 如果后续微信侧返回失败，后端会按已存在的提现支出流水返还钱包余额，并把提现记录更新为失败。
...
| status | 含义 | 说明 |
|--------|------|------|
| 0 | 打款中 | 已发起微信转账并已扣减钱包可用余额，等待到账确认 |
| 1 | 已打款 | 资金已到达用户微信零钱，余额不再重复扣减 |
| 2 | 打款失败 | 转账失败，可查看 failType 了解原因；若已扣款则已返还 |
...
- `code === 200 && data.status === 0 && data.packageInfo`：余额已扣减，先调用 `uni.requestMerchantTransfer`，用户确认后关闭弹窗并启动轮询（每 5s，最多 6 次）
- `code === 200 && data.status === 0 && !data.packageInfo`：余额已扣减，关闭弹窗，提示"提现处理中"，启动轮询（每 5s，最多 6 次）
