...
- 状态约定：`0=待上课`、`1=正在上课`、`2=待结算`、`3=已结算`。
- 后台审核通过后会自动发放扣佣后课酬到伴学管钱包，正常情况下课表会直接推进到 `3=已结算`。
- 当 `status = 1` 且 `confirmTime != null` 时，表示家长已确认，课表进入“待管理员审核”状态；前台不应再次展示家长确认按钮。
- `finishTime` / `finishRemark` 为历史字段名，当前用于表示学生上课签到时间与备注。
