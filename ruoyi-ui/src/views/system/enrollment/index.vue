<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="用户ID (关联小程序学员)" prop="uid">
        <el-input
          v-model="queryParams.uid"
          placeholder="请输入用户ID (关联小程序学员)"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="关联课程ID" prop="lectureId">
        <el-input
      ...
      <el-form-item label="总学籍数/报名数" prop="total">
        <el-input
          v-model="queryParams.total"
          placeholder="请输入总学籍数/报名数"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="剩余可用学籍数 (支持线下核销或赠送扣减)" prop="remain">
        <el-input
          v-model="queryParams.remain"
          placeholder="请输入剩余可用学籍数 (支持线下核销或赠送扣减)"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="enrollmentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="用户ID (关联小程序学员)" align="center" prop="uid" />
      <el-table-column label="关联课程ID" align="center" prop="lectureId" />
      <el-table-column label="总学籍数/报名数" align="center" prop="total" />
      <el-table-column label="剩余可用学籍数 (支持线下核销或赠送扣减)" align="center" prop="remain" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="用户ID (关联小程序学员)" prop="uid">
          <el-input v-model="form.uid" placeholder="请输入用户ID (关联小程序学员)" />
        </el-form-item>
        <el-form-item label="关联课程ID" prop="lectureId">
          <el-input v-model="form.lectureId" placeholder="请输入关联课程ID" />
        </el-form-item>
        <el-form-item label="总学籍数/报名数" prop="total">
          <el-input v-model="form.total" placeholder="请输入总学籍数/报名数" />
        </el-form-item>
        <el-form-item label="剩余可用学籍数 (支持线下核销或赠送扣减)" prop="remain">
          <el-input v-model="form.remain" placeholder="请输入剩余可用学籍数 (支持线下核销或赠送扣减)" />
        </el-form-item>
        <el-form-item label="逻辑删除标识" prop="isDeleted">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        uid: [
          { required: true, message: "用户ID (关联小程序学员)不能为空", trigger: "blur" }
        ],
        lectureId: [
          { required: true, message: "关联课程ID不能为空", trigger: "blur" }
        ],
        total: [
          { required: true, message: "总学籍数/报名数不能为空", trigger: "blur" }
        ],
        remain: [
          { required: true, message: "剩余可用学籍数 (支持线下核销或赠送扣减)不能为空", trigger: "blur" }
        ],
      }
    }
  },
