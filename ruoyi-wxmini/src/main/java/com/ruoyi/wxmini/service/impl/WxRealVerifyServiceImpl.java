...
public class WxRealVerifyServiceImpl implements IWxRealVerifyService {
    ...
    private static final String PERSISTENCE_MESSAGE = "实名认证信息保存失败，请稍后重试";
    ...
    protected WxRealVerifyResultVo toResult(Id2MetaVerifyResponseBody body) {
        if (body == null || !SUCCESS_CODE.equals(body.getCode())) {
            log.error("阿里云实名认证返回失败，code={}, requestId={}",
                    body == null ? null : body.getCode(), body == null ? null : body.getRequestId());
    ...
    private void persistVerifiedUser(String userId, String realName, String idCard) {
        try {
            ...
            log.error("保存实名认证信息异常，userId={}", userId, e);
            throw new ServiceException(PERSISTENCE_MESSAGE);
        }
    }
}
