...
public class WxCommonController {
    ...
    @PostMapping("/uploadMerchantLicense")
    public AjaxResult uploadMerchantLicense(MultipartFile file) {
        try {
            String userId = WxMiniUserContext.getCurrentUserId();
            if (StringUtils.isBlank(userId)) {
                return AjaxResult.error("请先登录");
            ...
            Path licenseDir = Paths.get(RuoYiConfig.getProfile(), "merchant-license", userId);
            Files.createDirectories(licenseDir);

            String fileName = System.currentTimeMillis() + "." + extension;
            Path target = licenseDir.resolve(fileName);
            file.transferTo(target.toFile());

            String resourcePath = "/profile/merchant-license/" + userId + "/" + fileName;
            AjaxResult ajax = AjaxResult.success();
            ajax.put("url", serverConfig.getUrl() + resourcePath);
            ajax.put("fileName", resourcePath);
            ajax.put("newFileName", fileName);
